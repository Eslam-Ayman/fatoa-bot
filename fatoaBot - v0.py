...
# SOURCE backup.sql;
import MySQLdb
db = MySQLdb.connect("localhost","ahmedreo4","","fatoaBot",use_unicode=True,charset='utf8')
myCursor = db.cursor()
...
def start (bot,update):
    ...
    bot.sendMessage(chat_id =update.message.chat_id , text = data_man[1] , reply_markup =r)

def answer(bot, update):
    db = MySQLdb.connect("localhost","ahmedreo4","","fatoaBot",use_unicode=True,charset='utf8')
    myCursor = db.cursor()
    keyboard = []
    if update.message.text in  data_man:
        myCursor.execute("UPDATE `chat` SET `man`='%s' WHERE `ch_id` = '%s'" % (update.message.text,update.message.chat_id))
        db.commit()
    ...
    elif update.message.text in data_department:
